...
class WSBDipDetector:
    ...
    def _analyze_recent_run(self, closes: List[Decimal], highs: List[Decimal]) -> Dict[str, Any]:
        ...
        try:
            ...
            run_duration=recent_high_idx - closes[lookback_start:lookback_end].index(base_price) - lookback_start
            
            # Validate run criteria
            valid_run=(
                run_percentage >= self.min_run_percentage and
                run_duration >= 1 and  # At least 1 day
                run_duration <= 15     # No more than 15 days
            )
    ...
    def _analyze_current_dip(self, closes: List[Decimal], highs: List[Decimal]) -> Dict[str, Any]:
        ...
        try:
            ...
            valid_dip=(
                dip_percentage >= self.min_dip_percentage and
                days_since_high <= self.max_dip_age_days and
                current_price < recent_high * Decimal('0.98')  # At least 2% below high
            )
    ...
    def _analyze_technical_indicators(self, closes: List[Decimal], volumes: List[int]) -> Dict[str, Any]:
    ...
    def _calculate_signal_strength(self, run_analysis: Dict, dip_analysis: Dict,
                                 volume_analysis: Dict, technical_analysis: Dict) -> int:
