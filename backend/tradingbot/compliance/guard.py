...
class SessionCalendar:
    ...
    def _tod(self, t: dt.time) -> dt.time:
        return t
    ...
    def session(self, when: dt.datetime) -> Literal["pre", "regular", "post", "closed"]:
        ...
        o = dt.time(*self.open_t)
        c = dt.time(*self.close_t)
        pre = dt.time(*self.pre_open)
        post = dt.time(*self.post_close)
        if pre <= t < o:
            return "pre"
        if o <= t < c:
            return "regular"
...
class ComplianceGuard:
    ...
    def __init__(
        ...
        self.luld_limits: Dict[
            str, Dict[str, float]
        ] = {}  # symbol -> {"lower":x,"upper":y}
    ...
    def record_day_trade(
        self, symbol: str, opened: dt.datetime, closed: dt.datetime
    ) -> None:
    ...
    def check_pdt(
        self, account_equity: float, pending_day_trades_count: int, now: dt.datetime
    ) -> None:
        ...
        if pending_day_trades_count >= 4:
            raise PDTViolation(
                "Pattern Day Trader rule: equity < $25k and >= 4 day trades in 5 days."
            )

    def check_session(
        self, now: dt.datetime, allow_pre: bool, allow_post: bool
    ) -> None:
        s = self.session_calendar.session(now)
        if s == "regular":
            return
        if s == "pre" and allow_pre:
            return
        if s == "post" and allow_post:
            return
    ...
    def check_luld(self, symbol: str, limit_price: Optional[float]) -> None:
        ...
        if band:
            if not (band["lower"] <= limit_price <= band["upper"]):
                raise HaltViolation(
                    f"{symbol} limit {limit_price} breaches LULD [{band['lower']}, {band['upper']}]"
                )
