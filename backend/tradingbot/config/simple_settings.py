...
class AppSettings:
    def __init__(self):
        ...
        # Analytics/Regime
        self.enable_advanced_analytics = os.getenv("ENABLE_ADV_ANALYTICS", "True").lower() == "true"
        self.enable_market_regime_adaptation = os.getenv("ENABLE_REGIME_ADAPT", "True").lower() == "true"
        self.analytics_update_interval = int(os.getenv("ANALYTICS_UPDATE_INTERVAL", "3600"))
        self.regime_adaptation_interval = int(os.getenv("REGIME_ADAPTATION_INTERVAL", "1800"))
    ...
    def _validate(self):
        ...
        if not (0.0 <= self.max_total_risk <= 1.0):
            errors.append(f"max_total_risk must be between 0 and 1, got {self.max_total_risk}")

        if not (0.0 <= self.max_position_size <= 1.0):
            errors.append(f"max_position_size must be between 0 and 1, got {self.max_position_size}")
