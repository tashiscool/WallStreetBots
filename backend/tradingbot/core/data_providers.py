                    ...
                    if response.status == 200:
                        ...
                        for event in data:
                            events.append(EarningsEvent(
                                ...
                                estimated_eps=float(event.get('epsEstimated', 0)) if event.get('epsEstimated') else None
                            ))
                    ...
                    if response.status == 200:
                        ...
                        for event in data:
                            events.append(EarningsEvent(
                                ...
                                time=event.get('time', 'AMC'),
                                expected_move=0.0,
                                actual_eps=float(event.get('epsActual', 0)) if event.get('epsActual') else None,
                                estimated_eps=float(event.get('epsEstimated', 0)) if event.get('epsEstimated') else None,
                                surprise=float(event.get('epsSurprise', 0)) if event.get('epsSurprise') else None
                            ))
