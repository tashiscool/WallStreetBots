...
class ConfigManager:
    ...
    def _merge_config(
        ...
        if "data_providers" in file_config:
            ...
            base_config.data_providers.iex_api_key = dp_config.get(
                "iex_api_key", base_config.data_providers.iex_api_key
            )
            base_config.data_providers.polygon_api_key = dp_config.get(
                "polygon_api_key", base_config.data_providers.polygon_api_key
            )
            base_config.data_providers.fmp_api_key = dp_config.get(
                "fmp_api_key", base_config.data_providers.fmp_api_key
            )
            base_config.data_providers.news_api_key = dp_config.get(
        ...
        if "broker" in file_config:
            ...
            base_config.broker.alpaca_api_key = broker_config.get(
                "alpaca_api_key", base_config.broker.alpaca_api_key
            )
            base_config.broker.alpaca_secret_key = broker_config.get(
                "alpaca_secret_key", base_config.broker.alpaca_secret_key
            )
            base_config.broker.alpaca_base_url = broker_config.get(
                "alpaca_base_url", base_config.broker.alpaca_base_url
            )
            base_config.broker.ibkr_host = broker_config.get(
                "ibkr_host", base_config.broker.ibkr_host
            )
            base_config.broker.ibkr_port = broker_config.get(
                "ibkr_port", base_config.broker.ibkr_port
            )
            base_config.broker.ibkr_client_id = broker_config.get(
                "ibkr_client_id", base_config.broker.ibkr_client_id
            )

        # Risk
        if "risk" in file_config:
            risk_config = file_config["risk"]
            base_config.risk.max_position_risk = risk_config.get(
                "max_position_risk", base_config.risk.max_position_risk
            )
            base_config.risk.max_total_risk = risk_config.get(
                "max_total_risk", base_config.risk.max_total_risk
            )
            base_config.risk.max_drawdown = risk_config.get(
                "max_drawdown", base_config.risk.max_drawdown
            )
            base_config.risk.max_correlation = risk_config.get(
                "max_correlation", base_config.risk.max_correlation
            )
            base_config.risk.account_size = risk_config.get(
                "account_size", base_config.risk.account_size
            )
            base_config.risk.default_commission = risk_config.get(
                "default_commission", base_config.risk.default_commission
            )
            base_config.risk.default_slippage = risk_config.get(
                "default_slippage", base_config.risk.default_slippage
            )

        # Trading
        if "trading" in file_config:
            trading_config = file_config["trading"]
            base_config.trading.universe = trading_config.get(
                "universe", base_config.trading.universe
            )
            base_config.trading.scan_interval = trading_config.get(
                "scan_interval", base_config.trading.scan_interval
            )
            base_config.trading.max_concurrent_trades = trading_config.get(
                "max_concurrent_trades", base_config.trading.max_concurrent_trades
            )
            base_config.trading.enable_paper_trading = trading_config.get(
                "enable_paper_trading", base_config.trading.enable_paper_trading
            )
            base_config.trading.enable_live_trading = trading_config.get(
                "enable_live_trading", base_config.trading.enable_live_trading
            )

        # Alerts
        if "alerts" in file_config:
            alert_config = file_config["alerts"]
            base_config.alerts.enable_slack = alert_config.get(
                "enable_slack", base_config.alerts.enable_slack
            )
            base_config.alerts.slack_webhook_url = alert_config.get(
                "slack_webhook_url", base_config.alerts.slack_webhook_url
            )
            base_config.alerts.enable_email = alert_config.get(
                "enable_email", base_config.alerts.enable_email
            )
            base_config.alerts.email_smtp_server = alert_config.get(
                "email_smtp_server", base_config.alerts.email_smtp_server
            )
            base_config.alerts.email_smtp_port = alert_config.get(
                "email_smtp_port", base_config.alerts.email_smtp_port
            )
            base_config.alerts.email_username = alert_config.get(
                "email_username", base_config.alerts.email_username
            )
            base_config.alerts.email_password = alert_config.get(
                "email_password", base_config.alerts.email_password
            )
            base_config.alerts.email_recipients = alert_config.get(
                "email_recipients", base_config.alerts.email_recipients
            )
    ...
    def _load_from_env(self, config: ProductionConfig) -> ProductionConfig:
        ...
        config.data_providers.polygon_api_key = os.getenv(
            "POLYGON_API_KEY", config.data_providers.polygon_api_key
        )
        config.data_providers.fmp_api_key = os.getenv(
            "FMP_API_KEY", config.data_providers.fmp_api_key
        )
        config.data_providers.news_api_key = os.getenv(
            "NEWS_API_KEY", config.data_providers.news_api_key
        )
        config.data_providers.alpha_vantage_api_key = os.getenv(
        ...
        config.broker.ibkr_client_id = int(
            os.getenv("IBKR_CLIENT_ID", config.broker.ibkr_client_id)
        )
        ...
        config.risk.default_slippage = float(
            os.getenv("DEFAULT_SLIPPAGE", config.risk.default_slippage)
        )
        ...
        config.trading.enable_live_trading = (
