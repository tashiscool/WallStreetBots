    ...
    async def _create_tables(self): 
        ...
        try: 
            async with self.pool.acquire() as conn: 
                ...
                await conn.execute("CREATE INDEX IF NOT EXISTS idx_trades_strategy_ticker ON trades(strategy_name, ticker)")
                await conn.execute("CREATE INDEX IF NOT EXISTS idx_trades_timestamp ON trades(created_at)")
                await conn.execute("CREATE INDEX IF NOT EXISTS idx_positions_strategy ON positions(strategy_name)")
                await conn.execute("CREATE INDEX IF NOT EXISTS idx_positions_active ON positions(is_active)")
