...
from .data_providers import OptionsData, UnifiedDataProvider
...
class SpreadPosition:
    ...
    def calculate_max_profit(self) -> float:
        ...
        if self.spread_type == SpreadType.BULL_CALL_SPREAD:
            ...
            ) * self.quantity * 100 - self.net_debit * self.quantity * 100
        return 0.0
    ...
    def calculate_black_scholes(
        ...
        try:
            ...
            if option_type.lower() == "call":
                price = spot_price * self._normal_cdf(d1) - strike_price * math.exp(
...
class ProductionDebitSpreads:
    ...
    def __init__(
        ...
        self.logger.info(
            "Debit Spreads Strategy initialized",
    ...
    async def execute_debit_spread(self, candidate: SpreadCandidate) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"Debit spread executed: {candidate.ticker}",
                long_strike=candidate.long_strike,
                short_strike=candidate.short_strike,
                net_debit=candidate.net_debit,
                max_profit=candidate.max_profit,
            ...
            self.error_handler.handle_error(
                e, {"ticker": candidate.ticker, "operation": "execute_spread"}
            )
            return False
    ...
    def _calculate_position_size(self, candidate: SpreadCandidate) -> int:
        ...
        try:
            ...
            self.error_handler.handle_error(
                e, {"ticker": candidate.ticker, "operation": "position_sizing"}
            )
            return 1
    ...
    async def manage_positions(self):
        ...
        for position_key, position in list(self.positions.items()):
            try:
                ...
                self.error_handler.handle_error(
                    e, {"position": position_key, "operation": "manage_position"}
                )
    ...
    async def _close_position(self, position: SpreadPosition, reason: str):
        ...
        try:
            ...
            if long_result.status.value == "filled" and short_result.status.value == "filled":
                ...
                self.logger.info(
                    f"Debit spread closed: {position.ticker}",
                    reason=reason,
...
def create_debit_spreads_strategy(
    trading_interface: TradingInterface,
    data_provider: UnifiedDataProvider,
    config: ProductionConfig,
    logger: ProductionLogger,
