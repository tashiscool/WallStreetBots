...
class EarningsDataProvider:
    ...
    async def get_earnings_history(self, ticker: str, quarters: int = 8) -> list[dict]:
        ...
        try:
            ...
            self.logger.info(
                f"Retrieved {len(mock_history)} quarters of earnings history for {ticker}"
            )
            return mock_history
...
class EarningsProtectionStrategy:
    ...
    def _check_position_risks(self, position: EarningsPosition) -> list[str]:
        ...
        if position.days_to_earnings <= 2:
            alerts.append(
                f"Earnings approaching for {position.ticker}: {position.days_to_earnings} days"
            )
    ...
    def _check_roll_opportunities(self, position: EarningsPosition) -> bool:
        ...
        return (
            ...
            and position.strategy != EarningsStrategy.VOLATILITY_ARBITRAGE
        )
    ...
    async def _close_position(self, ticker: str):
        ...
        try:
            if ticker in self.active_positions:
                ...
                self.logger.info(
                    f"Closed earnings position for {ticker}: P & L ${position.unrealized_pnl:.2f}"
                )
