...
class LEAPSCandidate:
    ...
    position_size: int
    recommended_option: LEAPSOption | None = None
...
class LEAPSOptionsProvider:
    ...
    def find_best_leaps_option(
        self,
        options: list[LEAPSOption],
        signal: LEAPSSignal,
        current_price: float,
        secular_trend: SecularTrend,
    ) -> LEAPSOption | None:
        ...
        try:
            ...
            if signal in [LEAPSSignal.STRONG_BUY, LEAPSSignal.BUY]:
                ...
                call_options = [
                    opt for opt in options if opt.option_type == LEAPSStrategy.LONG_CALL
                ]
                if call_options:
                    # Find option with strike closest to current price
                    best_option = min(
                        call_options, key=lambda x: abs(x.strike_price - current_price)
                    )
                    return best_option
                ...
                if put_options:
                    # Find option with strike closest to current price
                    best_option = min(
                        put_options, key=lambda x: abs(x.strike_price - current_price)
                    )
                    return best_option
...
class LEAPSTrackerStrategy:
    ...
    async def _create_leaps_candidate(
        self,
        ticker: str,
        secular_analysis: SecularAnalysis,
        signal: LEAPSSignal,
    ...
    def _check_position_risks(self, position: LEAPSPosition) -> list[str]:
        ...
        if position.days_to_expiry <= 60:
            alerts.append(
                f"LEAPS expiring soon for {position.ticker}: {position.days_to_expiry} days"
            )
        ...
        if months_held >= self.max_hold_months - 3:
            alerts.append(
                f"Approaching max hold months for {position.ticker}: {months_held:.1f} months"
            )
    ...
    async def _close_position(self, ticker: str, exit_signal: str):
        ...
        try:
            if ticker in self.active_positions:
                ...
                self.logger.info(
                    f"Closed LEAPS position for {ticker}: P & L ${position.unrealized_pnl:.2f}, Signal: {exit_signal}"
                )
