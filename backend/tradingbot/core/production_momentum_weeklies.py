...
class MomentumCandidate:
    ...
    position_size: int
    recommended_option: WeeklyOption | None = None
    last_update: datetime = field(default_factory=datetime.now)
...
class MomentumAnalyzer:
    ...
    def calculate_volume_momentum(self, volumes: list[int]) -> dict[str, float]:
        ...
        # 1 - day volume change
        volume_change_1d = (
            (current_volume - volumes[-2]) / volumes[-2]
        ...
        # 5 - day volume change
        volume_change_5d = (
            (current_volume - volumes[-6]) / volumes[-6]
    ...
    def calculate_moving_averages(self, prices: list[float]) -> dict[str, float]:
        ...
        return {"sma_20": sma_20, "sma_50": sma_50, "ema_12": ema_12, "ema_26": ema_26}
...
class MomentumWeekliesStrategy:
    ...
    async def _create_momentum_candidate(
        self,
        ticker: str,
        momentum_data: MomentumData,
        signal: MomentumSignal,
    ...
    def _check_position_risks(self, position: MomentumPosition) -> list[str]:
        ...
        if position.days_to_expiry <= 1:
            alerts.append(
                f"Option expiring soon for {position.ticker}: {position.days_to_expiry} days"
            )
    ...
    async def _close_position(self, ticker: str, exit_signal: str):
        ...
        try:
            if ticker in self.active_positions:
                ...
                self.logger.info(
                    f"Closed momentum position for {ticker}: P & L ${position.unrealized_pnl:.2f}, Signal: {exit_signal}"
                )
