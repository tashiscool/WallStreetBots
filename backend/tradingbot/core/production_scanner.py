...
def fetch_current_price(ticker: str) -> dict[str, float] | None:
    ...
    try:
        ...
        return {"current_price": current_price, "prior_close": prior_close}
    ...
    for ticker in universe:
        try:
            ...
            if signal:
                print(f"\n🚨 SIGNAL: {ticker}")
                print(
                    f"   Dip: {to_pct(signal.dip_percentage)} (${signal.spot_price: .2f} from ${signal.prior_close: .2f})"
                )
                print(
                    f"   Prior run: {to_pct(signal.run_return)} over {signal.run_lookback_days} days"
                )
            ...
            for ticker in universe:
                ...
                try:
                    ...
                    if signal:
                        ...
                        print(
                            f"   Live dip: {to_pct(signal.dip_percentage)} (${signal.spot_price: .2f} from ${signal.prior_close: .2f})"
                        )
                        print(
                            f"   Prior run: {to_pct(signal.run_return)} over {signal.run_lookback_days} days"
                        )
...
def write_results(results: ScanResults, output_prefix: str) -> None:
    ...
    summary = {
        ...
        "average_leverage": sum(p.effective_leverage for p in results.trade_plans)
        / len(results.trade_plans)
        if results.trade_plans
        else 0,
    }
  ...
  python production_scanner.py --mode eod --account - size 50000 --deploy - pct 0.10
    ...
    try:
        ...
        if results.trade_plans:
            ...
            avg_risk = sum(p.ruin_risk_percentage for p in results.trade_plans) / len(
                results.trade_plans
            )
            avg_leverage = sum(p.effective_leverage for p in results.trade_plans) / len(
                results.trade_plans
            )
