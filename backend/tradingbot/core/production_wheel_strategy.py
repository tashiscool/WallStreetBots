...
from .data_providers import MarketData, OptionsData, UnifiedDataProvider
from .production_config import ProductionConfig
from .production_logging import ErrorHandler, MetricsCollector, ProductionLogger
from .trading_interface import OrderSide, OrderType, TradeSignal, TradingInterface
...
class WheelPosition:
    ...
    def update_pricing(self, market_data: MarketData, options_data: list[OptionsData]):
        ...
        for option in options_data:
            if (
                option.strike == self.strike_price
                and option.option_type == self.option_type
                and option.expiry_date == self.expiry_date.strftime("%Y-%m-%d")
            ):
...
class ProductionWheelStrategy:
    ...
    async def execute_wheel_trade(self, candidate: WheelCandidate) -> bool:
        ...
        try:
            ...
            if trade_result.status.value == "filled":  # Create wheel position
                ...
                self.logger.info(
                    f"Wheel trade executed: {candidate.ticker}",
            ...
            self.error_handler.handle_error(
                e, {"ticker": candidate.ticker, "operation": "execute_trade"}
            )
            return False
    ...
    async def manage_positions(self):
        ...
        for ticker, position in list(self.positions.items()):
            try:
                ...
                self.error_handler.handle_error(
                    e, {"ticker": ticker, "operation": "manage_position"}
                )
    ...
    async def _sell_covered_call(self, position: WheelPosition, call_option: OptionsData):
        ...
        try:
            ...
            self.error_handler.handle_error(
                e, {"ticker": position.ticker, "operation": "sell_covered_call"}
            )
    ...
    async def _close_position(self, position: WheelPosition, reason: str):
        ...
        try:
            ...
            if trade_result.status.value == "filled":  # Update position
                ...
                self.logger.info(
                    f"Wheel position closed: {position.ticker}",
                    reason=reason,
            ...
            self.error_handler.handle_error(
                e, {"ticker": position.ticker, "operation": "close_position"}
            )
    ...
    async def _roll_position(self, position: WheelPosition, reason: str):
        ...
        try:
            ...
            self.error_handler.handle_error(
                e, {"ticker": position.ticker, "operation": "roll_position"}
            )
