...
class CorporateActionsAdjuster:
    ...
    def __init__(self, actions: list[CorporateAction]):
        try:
            self.splits = sorted([a for a in actions if a.kind == "split"], key=lambda x: x.date)
            self.divs = sorted([a for a in actions if a.kind == "div"], key=lambda x: x.date)
    ...
    def adjust(self, bars: pd.DataFrame, price_cols=("open","high","low","close")) -> pd.DataFrame:
        df = bars.copy()
        # Splits (back-adjust)
        adj_factor = 1.0
