...
class ExitStrategy:
    ...
    def analyze_exit_conditions(
        ...
        if current_iv < 0.15 and current_roi < 0.5:  # Low IV + modest gains
            signals.append(
                ExitSignal(
                    ...
                    expected_pnl=(current_theoretical_price - position.entry_premium)
                    * position.contracts
                    * 0.5,
                    reasoning=[
                        "Low implied volatility may limit upside",
        ...
        signals.sort(
            key=lambda x: (
                0
                if x.strength == ExitSignalStrength.URGENT
                else 1
                if x.strength == ExitSignalStrength.STRONG
                else 2
                if x.strength == ExitSignalStrength.MODERATE
                else 3
            )
        )
    ...
    def _check_exit_level(
        ...
        if exit_level.target_roi >= 0:  # Profit target
            if current_roi >= exit_level.target_roi:
                triggered = True
                reasoning.append(
                    f"Profit target hit: {current_roi:.1%} ≥ {exit_level.target_roi: .1%}"
                )
                strength = ExitSignalStrength.MODERATE
        elif current_roi <= exit_level.target_roi:
            triggered = True
            reasoning.append(
                f"Stop loss triggered: {current_roi:.1%} ≤ {exit_level.target_roi: .1%}"
            )
            strength = ExitSignalStrength.STRONG
        ...
        if exit_level.delta_threshold and current_delta >= exit_level.delta_threshold:
            triggered = True
            reasoning.append(
                f"Delta threshold reached: {current_delta:.3f} ≥ {exit_level.delta_threshold: .3f}"
            )
            strength = ExitSignalStrength.STRONG
        ...
        if exit_level.days_threshold and days_since_entry >= exit_level.days_threshold:
            triggered = True
            reasoning.append(
                f"Time threshold exceeded: {days_since_entry} days ≥ {exit_level.days_threshold} days"
            )
            strength = ExitSignalStrength.MODERATE
        ...
        if triggered:
            expected_pnl = (
                (current_price - position.entry_premium)
                * position.contracts
                * exit_level.position_fraction
            )

            return ExitSignal(
                reason=ExitReason.PROFIT_TARGET
                if exit_level.target_roi >= 0
    ...
    def generate_exit_plan(self, scenarios: list[ScenarioResult]) -> dict:
        ...
        plan = {
            "summary": {
                ...
                "win_rate": len(profitable_scenarios) / total_scenarios
...
if __name__ == "__main__":  # Test the exit planning system
