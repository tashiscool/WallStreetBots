"""Mean-Variance Optimization Portfolio Model"""
...
class MeanVariancePortfolioModel(PortfolioConstructionModel):
    ...
    def create_targets(
        ...
        # Optimal unconstrained: w* = (1/lambda) * inv(C) * mu
        try:
            inv_cov = np.linalg.inv(cov + np.eye(len(valid_insights)) * 1e-8)
        except np.linalg.LinAlgError:
            return self._equal_weight(insights, state)

        raw_weights = (1.0 / self.risk_aversion) * inv_cov @ mu
