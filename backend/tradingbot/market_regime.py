...
class MarketRegimeFilter:
    ...
    def detect_pullback_setup(
        ...
        near_20ema = (
            abs(indicators.distance_from_20ema) <= self.max_distance_from_20ema
            or indicators.low_24h <= indicators.ema_20
        )
...
class SignalGenerator:
    ...
    def generate_signal(
        ...
        if has_pullback_setup:
            reasoning.append(
                "Pullback setup detected: red day into 20 - EMA support with RSI 35 - 50"
            )
...
if __name__ == "__main__":  # Test the regime filter and signal generator
    ...
    prev_indicators = create_sample_indicators(
        price=210.0, ema_20=208.0, ema_50=205.0, ema_200=200.0, rsi=45, volume=1500000
    )
    ...
    bear_indicators = create_sample_indicators(
        price=195.0, ema_20=198.0, ema_50=202.0, ema_200=205.0, rsi=35, volume=1000000
    )
