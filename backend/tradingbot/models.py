...
class News(models.Model):
    ...
    # Methods
    def __str__(self):
        return f"Headline: {self.headline!s} \n Link: {self.link} \n Date: {self.date}"
...
class Tweets(models.Model):
    ...
    # Methods
    def __str__(self):
        return f"Content: {self.content!s} \n Date: {self.date}"
...
class Order(models.Model):
    ...
    STATUS = [("A", "Accepted"), ("F", "Filled"), ("N", "New"), ("C", "Closed")]
    # Fields
    order_number = models.BigAutoField(primary_key=True)
    client_order_id = models.CharField(
        max_length=100, default="", help_text="for alpaca sync"
    )
    user = models.ForeignKey(
        User, help_text="associated user", on_delete=models.CASCADE
    )
    timestamp = models.DateTimeField(
        auto_now_add=True, help_text="order submission timestamp"
    )
    stock = models.ForeignKey(
        Stock, help_text="associated stock", on_delete=models.CASCADE
    )
    order_type = models.CharField(
        choices=ORDERTYPES, max_length=2, help_text="order type"
    )
    quantity = models.DecimalField(max_digits=8, decimal_places=2, help_text="quantity")
    transaction_type = models.CharField(
        choices=TRANSACTIONTYPES, max_length=2, help_text="buy or sell transaction type"
    )
    ...
    filled_timestamp = models.DateTimeField(
        blank=True, help_text="order filled timestamp", null=True
    )
    limit_price = models.DecimalField(
        max_digits=8, decimal_places=2, help_text="limit price", blank=True, null=True
    )
    stop_price = models.DecimalField(
        max_digits=8, decimal_places=2, help_text="stop price", blank=True, null=True
    )
    filled_quantity = models.DecimalField(
        max_digits=8, decimal_places=2, default=0, help_text="filled quantity"
    )
...
class StockInstance(models.Model):
    ...
    def __str__(self):
        return (
            f"User: {self.user!s} Stock: {self.stock!s} \n Quantity: {self.quantity} \n "
