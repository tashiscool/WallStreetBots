...
class SystemHealthMonitor:
    ...
    async def check_system_health(self) -> SystemHealthReport:
        ...
        try:
            ...
            broker_health = health_checks[1] if not isinstance(health_checks[1], Exception) else self._create_error_health("broker", health_checks[1])
            db_health = health_checks[2] if not isinstance(health_checks[2], Exception) else self._create_error_health("database", health_checks[2])
            resource_health = health_checks[3] if not isinstance(health_checks[3], Exception) else self._create_error_health("resources", health_checks[3])
            trading_health = health_checks[4] if not isinstance(health_checks[4], Exception) else self._create_error_health("trading", health_checks[4])
            
            # Calculate overall health
            overall_status = self._calculate_overall_health([
                data_feed_health, broker_health, db_health, resource_health, trading_health
            ])
    ...
    async def _check_database_performance(self) -> ComponentHealth:
        ...
        try:
            ...
            status = HealthStatus.HEALTHY if response_time < 1000 else HealthStatus.DEGRADED  # 1 second threshold
    ...
    def _create_critical_report(self, error_message: str) -> SystemHealthReport:
        ...
        return SystemHealthReport(
            ...
            data_feed_status=self._create_error_health("data_feed", Exception(error_message)),
            broker_status=self._create_error_health("broker", Exception(error_message)),
            database_status=self._create_error_health("database", Exception(error_message)),
            resource_status=self._create_error_health("resources", Exception(error_message)),
            trading_status=self._create_error_health("trading", Exception(error_message)),
