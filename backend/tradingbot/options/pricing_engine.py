...
if not SCIPY_AVAILABLE:
    logger.warning(
        "Using fallback math implementations - consider installing scipy and numpy for better performance"
    )
...
class RealOptionsPricingEngine:
    ...
    async def calculate_theoretical_price(
        self,
        ticker: str,
        strike: Decimal,
        expiry_date: date,
        ...
        try:
            ...
            logger.error(
                f"Error calculating theoretical price for {ticker} {strike} {option_type}: {e}"
            )
