...
class BlackScholesCalculator:
    ...
    def call_price(
        ...
        call_value = spot * math.exp(
            -dividend_yield * time_to_expiry_years
        ) * BlackScholesCalculator._norm_cdf(d1) - strike * math.exp(
    ...
    def put_price(
        ...
        put_value = strike * math.exp(
            -risk_free_rate * time_to_expiry_years
        ) * BlackScholesCalculator._norm_cdf(-d2) - spot * math.exp(
            -dividend_yield * time_to_expiry_years
    ...
    def vega(
        ...
        return (
            ...
            * math.sqrt(time_to_expiry_years)
            / 100  # Convert to 1% volatility change
        )
...
class OptionsStrategySetup:
    ...
    # Universe filter
    mega_cap_tickers: list[str] = field(
...
class OptionsTradeCalculator:
    ...
    def find_optimal_expiry(self, target_dte: int | None = None) -> date:
    ...
    def calculate_trade(
        ...
        if not (0 < risk_pct <= self.setup.max_single_trade_risk_pct):
            raise ValueError(
                f"Risk percentage must be between 0 and {self.setup.max_single_trade_risk_pct}"
            )
        ...
        for move in spot_moves:
            ...
            try:
                ...
                roi = (
                    ...
                    if trade_calc.estimated_premium > 0
                    else 0
                )
