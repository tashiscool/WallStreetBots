...
class Phase3StrategyManager:
    ...
    async def scan_all_opportunities(self) -> Dict[str, List[Any]]:
        ...
        try:
            ...
            try:
                earnings_opportunities = await self.earnings_protection.scan_for_earnings_opportunities()
                opportunities["earnings_protection"] = earnings_opportunities
                self.logger.info(f"Found {len(earnings_opportunities)} earnings protection opportunities")
            except Exception as e:
            ...
            try:
                momentum_opportunities=await self.momentum_weeklies.scan_for_momentum_opportunities()
                opportunities["momentum_weeklies"] = momentum_opportunities
                self.logger.info(f"Found {len(momentum_opportunities)} momentum weeklies opportunities")
            except Exception as e:
    ...
    async def execute_strategy_trades(self, opportunities: Dict[str, List[Any]]) -> Dict[str, List[Any]]:
            ...
            if "earnings_protection" in opportunities:
                try:
                    ...
                    for candidate in opportunities["earnings_protection"][:3]:  # Limit to top 3
                        trade = await self.earnings_protection.execute_earnings_protection(candidate)
                        if trade:
