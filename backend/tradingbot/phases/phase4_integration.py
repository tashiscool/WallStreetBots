...
from .data_providers import UnifiedDataProvider, create_data_provider
...
from .phase4_production import (
    ...
    create_production_backtest_engine,
)
...
from .trading_interface import TradingInterface, create_trading_interface
...
class Phase4IntegrationManager:
    ...
    def __init__(self, config_file_path: str | None = None):
        ...
        # System components
        self.database: ProductionDatabaseManager | None = None
        self.trading_interface: TradingInterface | None = None
        self.data_provider: UnifiedDataProvider | None = None

        # Phase managers
        self.phase2_manager: Phase2StrategyManager | None = None
        self.phase3_manager: Phase3StrategyManager | None = None
        self.backtest_engine: ProductionBacktestEngine | None = None
        self.high_risk_orchestrator: HighRiskStrategyOrchestrator | None = None
    ...
    async def initialize_system(self) -> bool:
        ...
        try:
            self.logger.info(
                "🚀 Initializing complete WallStreetBots production system..."
            )
            self.system_status = SystemStatus.INITIALIZING
    ...
    async def _initialize_data_provider(self) -> bool:
        ...
        try:
            ...
            self.error_handler.handle_error(
                e, {"operation": "_initialize_data_provider"}
            )
            return False
    ...
    async def _initialize_trading_interface(self) -> bool:
        ...
        try:
            ...
            if account_info:
                self.logger.info(
                    "✅ Trading interface initialized and tested successfully"
                )
                return True
            else:
            ...
            self.error_handler.handle_error(
                e, {"operation": "_initialize_trading_interface"}
            )
            return False
    ...
    async def _initialize_phase_managers(self) -> bool:
        ...
        try:
            ...
            self.error_handler.handle_error(
                e, {"operation": "_initialize_phase_managers"}
            )
            return False
    ...
    async def _initialize_phase4_components(self) -> bool:
        ...
        try:
            ...
            self.backtest_engine = create_production_backtest_engine(
                self.config, self.database
            )

            # Initialize high - risk orchestrator
            self.high_risk_orchestrator = create_high_risk_orchestrator(
                self.config, self.database
            )
            ...
            self.error_handler.handle_error(
                e, {"operation": "_initialize_phase4_components"}
            )
            return False
        ...
        try:
            ...
            for strategy_name, allocation in self.strategy_allocations.items():
                ...
                try:
                    ...
                    if strategy_passed:
                        ...
                        self.logger.info(
                            f"✅ Strategy validation PASSED: {strategy_name}"
                        )
            ...
            for strategy, result in validation_summary.items():
                if result == "PASSED":
                    ...
                    self.logger.info(
                        f"   ✅ {strategy}: {backtest.annualized_return:.2%} return, "
            ...
            if validation_passed:
                self.logger.info(
                    "🎉 ALL STRATEGIES VALIDATED SUCCESSFULLY - READY FOR PRODUCTION"
                )
                self.system_status = SystemStatus.READY
            else:
                self.logger.error(
                    "⚠️ STRATEGY VALIDATION FAILED - SYSTEM NOT READY FOR PRODUCTION"
                )
                self.system_status = SystemStatus.ERROR
        ...
        try:
            ...
            checks.append(
                (
                    "Sharpe Ratio",
                    sharpe_ok,
                    f"{results.sharpe_ratio: .2f}  >=  {allocation.min_sharpe_ratio: .2f}",
                )
            )
            ...
            checks.append(
                (
                    "Max Drawdown",
                    drawdown_ok,
                    f"{results.max_drawdown: .2%}  <=  {allocation.max_drawdown: .2%}",
                )
            )
            ...
            checks.append(
                ("Win Rate", winrate_ok, f"{results.win_rate: .1%}  >=  30.0%")
            )
            ...
            checks.append(
                ("Alpha vs SPY", alpha_ok, f"{results.alpha: .2%}  >=  0.00%")
            )
            ...
            self.error_handler.handle_error(
                e,
                {
                    ...
                    "operation": "_evaluate_strategy_performance",
                },
            )
            return False
    ...
    async def start_production_trading(self, paper_trading: bool = True) -> bool:
        ...
        try:
            self.logger.info(
                f"🚀 Starting production trading (paper_trading={paper_trading})..."
            )

            # Safety check: Ensure system is ready
            if self.system_status != SystemStatus.READY:
                raise Exception(
                    f"System not ready for trading. Status: {self.system_status}"
                )
            ...
            for strategy_name, allocation in self.strategy_allocations.items():
                if (
                    not allocation.enabled
                    or strategy_name not in self.validation_results
                ):
                    continue
            ...
            self.logger.info(
                f"🎉 Production system started successfully in {mode_str} mode"
            )
            self.logger.info(f"   Active strategies: {len(self.active_strategies)}")
            self.logger.info(
                f"   Total allocation: {sum(a.allocation_percentage for a in self.strategy_allocations.values() if a.enabled):.1%}"
            )
            ...
            self.error_handler.handle_error(
                e, {"operation": "start_production_trading"}
            )
            self.system_status = SystemStatus.ERROR
            return False
    ...
    async def _start_strategy(
        ...
        try:
            ...
            self.error_handler.handle_error(
                e, {"strategy": strategy_name, "operation": "_start_strategy"}
            )
            raise
            ...
            while self.system_status == SystemStatus.RUNNING:
                try:
                    ...
                    if health_check.overall_status == SystemStatus.RUNNING:
                        self.logger.debug(
                            f"System healthy: {health_check.active_strategies} active strategies, "
                            f"Daily P & L: {health_check.daily_pnl:.2f}"
                        )
                    else:
                        self.logger.warning(
                            f"System health issue: {health_check.overall_status}"
                        )
                    ...
                    self.metrics.record_metric(
                        "total_account_risk", float(health_check.total_account_risk)
                    )
                    self.metrics.record_metric(
    ...
    async def perform_system_health_check(self) -> SystemHealthCheck:
        ...
        try:
            ...
            health_check.active_strategies = len(
                [s for s in self.active_strategies.values() if not s["task"].done()]
            )
            ...
            # Daily loss check
            if health_check.daily_pnl < -self.config.risk.account_size * Decimal(
                "0.03"
            ):  # 3% daily loss
                health_check.alerts.append(
                    f"Significant daily loss: {health_check.daily_pnl:.2f}"
                )

            # Overall system status
            if (
                health_check.database_status
                and health_check.trading_interface_status
                and health_check.data_provider_status
                and health_check.active_strategies > 0
            ):
            ...
            self.error_handler.handle_error(
                e, {"operation": "perform_system_health_check"}
            )
            return SystemHealthCheck(
                overall_status=SystemStatus.ERROR, alerts=["Health check system error"]
            )
            ...
            for strategy_name, strategy_info in self.active_strategies.items():
                try:
                    ...
                    try:
                        ...
                        self.logger.warning(
                            f"Strategy {strategy_name} did not stop gracefully"
                        )
    ...
    async def get_system_summary(self) -> dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "allocations": {
                    name: float(alloc.allocation_percentage)
