...
from .phase4_backtesting import BacktestConfig, BacktestEngine, BacktestResults
...
class StrategyOptimizer:
    ...
    async def optimize_strategy(
        ...
        try:
            ...
            self.logger.info(
                f"Optimization completed: {len(all_results)} iterations, best score: {best_score:.4f}"
            )
            return result
        ...
        for i, params in enumerate(parameter_combinations):
            try:
                ...
                backtest_results = await self.backtest_engine.run_backtest(
                    strategy, config.backtest_config
                )
