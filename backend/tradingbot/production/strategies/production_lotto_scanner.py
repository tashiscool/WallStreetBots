...
class ProductionLottoScanner: 
    ...
    async def _scan_opportunities(self): 
        ...
        try: 
            ...
            if (best_opp.win_probability  >=  self.min_win_probability and 
                best_opp.risk_score  <=  8.0):  # Max risk score of 8
                ...
                try: 
                    ...
                    for option_type, target_strike in [("call", call_strike), ("put", put_strike)]: 
                        opp = await self._evaluate_option_opportunity(
                            ...
                            "0dte", f"Intraday momentum ({momentum_score})", 
            ...
            for event in earnings_events: 
                try: 
                    ...
                    for option_type, target_strike in [("call", call_strike), ("put", put_strike)]: 
                        opp = await self._evaluate_option_opportunity(
                            ticker, expiry, target_strike, option_type,
                            "earnings", catalyst, expected_move, current_price
                        )
                    ...
                    self.logger.error(f"Error scanning earnings for {event.get('ticker', 'unknown')}: {e}")
                    continue
            ...
            for ticker in self.meme_tickers: 
                try: 
                    ...
                    if volume_ratio  >  3.0 and abs(price_momentum)  >  0.03:  # 3x volume+3% move
                        ...
                        opp = await self._evaluate_option_opportunity(
                            ticker, expiry, target_strike, option_type,
                            "catalyst", catalyst, expected_move, current_price
                        )
    ...
    async def _evaluate_option_opportunity(self, ticker: str, expiry: str, target_strike: float,
                                         option_type: str, play_type: str, catalyst: str,
                                         expected_move: float, current_spot: float)->Optional[LottoOpportunity]:
    ...
    async def _execute_lotto_trade(self, opportunity: LottoOpportunity):
        ...
        try: 
            ...
            if success: 
                ...
                self.logger.info(f"Executed lotto trade: {opportunity.ticker} "
    ...
    async def _close_position(self, position_id: str, reason: str, close_pct: float=1.0):
        ...
        try: 
            ...
            if success: 
                ...
                self.logger.info(f"Closed {close_pct: .0%} of lotto position {position_id}: {reason}")
    ...
    async def _get_option_price(self, ticker: str, expiry: str, strike: float, option_type: str)->Optional[float]:
        ...
        try: 
            ...
            options_list = chain.get('calls', []) if option_type ==  'call' else chain.get('puts', [])
...
def create_production_lotto_scanner(integration_manager: ProductionIntegrationManager, 
                                  data_provider: ReliableDataProvider, 
                                  config: dict)->ProductionLottoScanner:
