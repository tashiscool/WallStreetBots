            ...
            try:
                ...
                for expiry, dte in expiries[:2]:  # Limit to 2 nearest expiries
                    ...
                    if put_short_strike and put_short_premium > 0:
                        ...
                        spread_width = min(
                            self.max_spread_width, max(self.min_spread_width, current_price * 0.03)
                        )
    ...
    async def manage_positions(self):
        ...
        for i, position in enumerate(self.active_positions):
            try:
                ...
                time_to_expiry = (
                    datetime.combine(position["expiry_date"], datetime.min.time()) - current_time
                )
    ...
    def get_strategy_status(self) -> dict[str, Any]:
        ...
        try:
            ...
            for position in self.active_positions:
                time_to_expiry = (
                    datetime.combine(position["expiry_date"], datetime.min.time()) - datetime.now()
                )
