...
class ProductionSPXCreditSpreads:
    ...
    def __init__(
        ...
        self.credit_tickers = config.get(
            "watchlist",
            [
        ...
        self.time_exit_minutes = config.get(
    ...
    async def find_target_delta_strike(
        ...
        try:
            ...
            for option in options_list:
                ...
                if bid > 0.05 and ask > 0.05 and (volume >= 10 or open_interest >= 50):
            ...
            for option in liquid_options:
                ...
                if "delta" in option and option["delta"] is not None:
                    ...
                    if option_type == "put":
                        _, delta = self.black_scholes_put(
                            spot_price, strike, time_to_exp, 0.04, iv_estimate
                        )
                        actual_delta = abs(delta)
                    else:
                        _, delta = self.black_scholes_call(
                            spot_price, strike, time_to_exp, 0.04, iv_estimate
                        )
                        actual_delta = abs(delta)
    ...
    async def scan_credit_spread_opportunities(self) -> list[CreditSpreadOpportunity]:
        ...
        for ticker in self.credit_tickers:
            try:
                ...
                self.logger.info(
                    f"Analyzing {ticker}: ${current_price:.2f}, expected move: ±{expected_move:.1%}"
                )
                ...
                for expiry, dte in expiries[:2]:  # Limit to 2 nearest expiries
                    ...
                    (
                        ...
                        put_short_delta,
                        put_short_premium,
                    ) = await self.find_target_delta_strike(
                        ticker, expiry, "put", self.target_short_delta, current_price
                    )

                    if put_short_strike and put_short_premium > 0:
                        # Calculate spread width (2 - 5% of underlying)
                        spread_width = min(
                            self.max_spread_width,
                    ...
                    (
                        ...
                        ticker, expiry, "call", self.target_short_delta, current_price
                    )
        ...
        opportunities.sort(
            key=lambda x: (x.prob_profit / 100.0)
            * (x.max_profit / max(x.max_loss, 0.01)),
            reverse=True,
        )
    ...
    async def execute_credit_spread(self, opportunity: CreditSpreadOpportunity) -> bool:
        ...
        try:
            ...
            short_signal = ProductionTradeSignal(
                ...
                premium=Decimal(
                    str(opportunity.net_credit * 0.6)
                ),  # Approximate short leg premium
            ...
            long_signal = ProductionTradeSignal(
                ...
                premium=Decimal(
                    str(opportunity.net_credit * 0.4)
                ),  # Approximate long leg premium
                confidence=min(1.0, opportunity.prob_profit / 100.0),
                strategy_name=self.strategy_name,
                signal_strength=min(1.0, opportunity.prob_profit / 100.0),
                metadata={
            ...
            if short_success and long_success:
                ...
                position = {
                    ...
                    "cost_basis": -opportunity.net_credit
                    * contracts
                    * 100,  # Negative for credit received
            ...
            self.logger.error(
                f"Error executing credit spread for {opportunity.ticker}: {e}"
            )
            return False
    ...
    async def manage_positions(self):
        ...
        for i, position in enumerate(self.active_positions):
            try:
                ...
                time_to_expiry = (
                    datetime.combine(position["expiry_date"], datetime.min.time())
                    - current_time
                )
                ...
                if should_exit:
                    ...
                    if short_exit_success and long_exit_success:
                        ...
                        self.logger.info(
                            f"Credit spread closed: {ticker} {exit_reason}"
                        )
            ...
            for opportunity in opportunities[:max_new_positions]:
                ...
                if success:
                    ...
                    trade_signal = ProductionTradeSignal(
                        ...
                        option_type="PUT"
                        if "put" in opportunity.strategy_type
                        else "CALL",
                        strike_price=Decimal(str(opportunity.short_strike)),
                        expiration_date=datetime.strptime(
                            opportunity.expiry_date, "%Y-%m-%d"
    ...
    def get_strategy_status(self) -> dict[str, Any]:
        ...
        try:
            total_credit_received = sum(
                -pos["cost_basis"] for pos in self.active_positions
            )
            position_details = []
