...
class PositionDiscrepancy:
    ...
    ticker: str
    db_position: PositionSnapshot | None
...
class PositionReconciler:
    ...
    async def reconcile_all_positions(self, auto_halt: bool = True) -> ReconciliationReport:
        ...
        try:
            ...
            self.logger.info(
                f"Retrieved {len(db_positions)} DB positions, {len(broker_positions)} broker positions"
            )
        ...
        for ticker in set(db_by_ticker.keys()) & set(broker_by_symbol.keys()):
            ...
            if abs(db_pos.quantity - broker_pos.quantity) > self.quantity_tolerance:
                severity = (
                    ...
                    if abs(db_pos.quantity - broker_pos.quantity) > 10
                ...
                discrepancy = PositionDiscrepancy(
                    ...
                    financial_impact=abs(
                        (db_pos.quantity - broker_pos.quantity) * db_pos.current_price
                    ),
                    timestamp=datetime.now(),
                    metadata={
    ...
    def _should_emergency_halt(self, report: ReconciliationReport) -> bool:
        ...
        missing_positions = sum(
            ...
            in [DiscrepancyType.MISSING_AT_BROKER, DiscrepancyType.MISSING_IN_DB]
        )
    ...
    async def _trigger_emergency_halt(
        ...
        try:
            halt_reason = (
                custom_reason
                or f"Position reconciliation found {report.critical_discrepancies} critical discrepancies"
            )
    ...
    def _log_reconciliation_results(self, report: ReconciliationReport):
        ...
        if report.is_clean:
            ...
            self.logger.warning(
                f"Position reconciliation found {report.total_discrepancies} discrepancies "
    ...
    async def get_reconciliation_summary(self) -> dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "next_reconciliation": latest_report.next_reconciliation.isoformat()
                if latest_report.next_reconciliation
                else None,
            }
