        ...
        try:
            ...
            if current_var > max_var:
                ...
                if action.action_type in [
                    RiskActionType.DECREASE_POSITION,
...
class PPORiskAgent:
    ...
    def update_policy(
        ...
        try:
            ...
            self.logger.info(
                f"Policy updated - Policy loss: {policy_loss:.4f}, "
...
class DDPGRiskAgent:
    ...
    def update_networks(
        ...
        try:
            ...
            self.logger.info(
                f"DDPG updated - Actor loss: {actor_loss:.4f}, Critic loss: {critic_loss:.4f}"
            )
...
class MultiAgentRiskCoordinator:
    ...
    def _make_ensemble_decision(
        ...
        try:
            ...
            ensemble_confidence = (
                action_votes[best_action_type] / total_votes if total_votes > 0 else 0.0
            )
        ...
        try:
            ...
            for agent_name, performance_history in self.agent_performance.items():
                if performance_history:
                    ...
                    recent_performance = (
                        np.mean(performance_history[-10:])
                        if len(performance_history) >= 10
                        else np.mean(performance_history)
                    )
            ...
            return dict.fromkeys(self.agents.keys(), 1.0)
