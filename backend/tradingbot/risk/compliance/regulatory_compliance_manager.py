...
class RegulatoryComplianceManager:
    ...
    def __init__(
        ...
        self.logger.info(
            f"Regulatory Compliance Manager initialized for {primary_authority}"
        )
    ...
    async def run_compliance_checks(
        ...
        try:
            ...
            for rule in self.compliance_rules.values():
                if rule.is_active:
                    check = await self._run_compliance_check(
                        rule, portfolio_data, risk_metrics
                    )
                    if check:
                        checks.append(check)
            ...
            self.logger.info(
                f"Ran {len(checks)} compliance checks, {self.violation_count} violations"
            )
    ...
    async def _run_compliance_check(
        ...
        try:
            ...
            if rule.rule_type in (
                ComplianceRule.POSITION_LIMITS,
                ComplianceRule.RISK_LIMITS,
            ):
    ...
    async def get_compliance_summary(self) -> dict[str, Any]:
        ...
        try:
            ...
            critical_violations = [
                c for c in recent_checks if c.status == ComplianceStatus.CRITICAL
            ]
            ...
            summary = {
                ...
                "active_rules": len(
                    [r for r in self.compliance_rules.values() if r.is_active]
                ),
                "checks_run": self.check_count,
                "violations": self.violation_count,
                "last_check": self.last_compliance_check.isoformat()
            ...
            for rule in self.compliance_rules.values():
                if rule.rule_type == ComplianceRule.POSITION_LIMITS:
                    if concentration_risk > rule.threshold:
                        violations.append(
                            f"Position concentration {concentration_risk: .2%} exceeds limit {rule.threshold: .2%}"
                        )
            ...
            if violations:
                ...
                if concentration_risk > 0.3:
                    remediation_actions.append(
                        "URGENT: Position exceeds 30% - immediate action required"
                    )
...
if __name__ == "__main__":
    ...
    async def test_regulatory_compliance():
        ...
        compliance_manager = RegulatoryComplianceManager(
            primary_authority=RegulatoryAuthority.FCA, enable_audit_trail=True
        )
