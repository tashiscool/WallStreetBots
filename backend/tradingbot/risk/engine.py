...
class RiskEngine:
    ...
    def var_cvar(self, pnl_samples: Sequence[float], alpha: float = 0.95) -> Dict[str, float]:
    ...
    def pretrade_check(self, current_exposure: float, new_position_risk: float) -> bool:
        ...
        if current_exposure + new_position_risk > self.limits.max_total_risk:
            log.warning(f"Total risk limit exceeded: {current_exposure + new_position_risk:.2%} > {self.limits.max_total_risk:.2%}")
            return False

        if new_position_risk > self.limits.max_position_size:
            log.warning(f"Position size limit exceeded: {new_position_risk:.2%} > {self.limits.max_position_size:.2%}")
            return False
    ...
    def posttrade_check(self, equity: float) -> bool:
        ...
        if dd >= self.limits.kill_switch_dd:
            log.critical(f"Kill switch triggered! Drawdown {dd:.2%} >= {self.limits.kill_switch_dd:.2%}")
            self._kill_switch_active = True
            return False

        if dd >= self.limits.max_drawdown:
            log.error(f"Maximum drawdown exceeded: {dd:.2%} >= {self.limits.max_drawdown:.2%}")
            return False
