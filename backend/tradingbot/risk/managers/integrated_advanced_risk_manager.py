...
class IntegratedAdvancedRiskManager:
    ...
    def _create_risk_state(
        ...
        try:
            ...
            return RiskState(
                ...
                recent_performance=np.mean(portfolio_returns[-10:])
            ...
            return RiskState(
                ...
                time_of_day=0.5,
                day_of_week=1,
                recent_performance=0.001,
                stress_test_score=0.7,
                ml_risk_score=50.0,
                position_count=5,
        ...
        while self.system_status == "monitoring":
            try:
                ...
                if self.config.enable_auto_rebalancing and (
                    self.last_rebalancing is None
                    or datetime.now() - self.last_rebalancing
                    > timedelta(seconds=self.config.rebalancing_frequency)
                ):
    ...
    async def _check_rebalancing_needs(
        ...
        try:
            ...
            if total_var > self.config.max_total_var:
                self.logger.warning(
                    f"Portfolio VaR {total_var: .3f} exceeds limit {self.config.max_total_var: .3f}"
                )
    ...
    def get_system_status(self) -> dict[str, Any]:
        ...
        return {
            ...
            "config": {
                "ml_agents_enabled": self.config.enable_ml_agents
                and self.ml_coordinator is not None,
                "multi_asset_enabled": self.config.enable_multi_asset
                and self.multi_asset_manager is not None,
                "compliance_enabled": self.config.enable_compliance
                and self.compliance_manager is not None,
