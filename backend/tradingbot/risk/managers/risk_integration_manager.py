...
from ..engines.advanced_var_engine import AdvancedVaREngine
from ..database_schema import RiskDatabaseManager
from ..ml_risk_predictor import MLRiskPredictor
from ..monitoring.risk_dashboard import RiskDashboard2025
from ..engines.stress_testing_engine import StressTesting2025
...
class RiskIntegrationManager:
    ...
    async def calculate_portfolio_risk(
        ...
        try:
            ...
            self.logger.info(
                f"Portfolio risk calculated: VaR={var_metrics['var_99']:.2%}, "
    ...
    async def _calculate_var_metrics(
        ...
        try:
            ...
            param_var_95 = var_parametric(
                portfolio_returns, 0.95, use_student_t=True, cornish_fisher=True
            )
            param_var_99 = var_parametric(
                portfolio_returns, 0.99, use_student_t=True, cornish_fisher=True
            )
            ...
            if len(portfolio_returns) >= 30:
                ...
                mc_var_95, mc_cvar_95 = var_cvar_mc(
                    mu, cov, weights, 0.95, n_paths=10000, student_t=True
                )
                mc_var_99, mc_cvar_99 = var_cvar_mc(
                    mu, cov, weights, 0.99, n_paths=10000, student_t=True
                )
            else:
    ...
    async def _calculate_stress_score(
        self,
        positions: dict[str, dict],
        market_data: dict[str, pd.DataFrame],
        portfolio_value: float,
    ...
    async def _calculate_ml_risk_score(
        ...
        try:
            ...
            risk_forecast = await self.ml_predictor.predict_risk(data, horizon_days=5)
    ...
    def _check_risk_limits(
        ...
        if var_metrics["var_99"] > self.risk_limits.max_total_var:
            alerts.append(
                f"VaR {var_metrics['var_99']: .2%} exceeds limit {self.risk_limits.max_total_var: .2%}"
            )
            within_limits = False
        ...
        if var_metrics["cvar_99"] > self.risk_limits.max_total_cvar:
            alerts.append(
                f"CVaR {var_metrics['cvar_99']: .2%} exceeds limit {self.risk_limits.max_total_cvar: .2%}"
            )
            within_limits = False
        ...
        if concentration_risk > self.risk_limits.max_concentration:
            alerts.append(
                f"Concentration {concentration_risk: .2%} exceeds limit {self.risk_limits.max_concentration: .2%}"
            )
            within_limits = False
        ...
        if greeks_risk > self.risk_limits.max_greeks_risk:
            alerts.append(
                f"Greeks risk {greeks_risk: .2%} exceeds limit {self.risk_limits.max_greeks_risk: .2%}"
            )
            within_limits = False
    ...
    async def get_risk_adjusted_position_size(
        ...
        try:
            ...
            if (
                ...
                self.logger.info(
                    f"Reducing position size due to high portfolio VaR: {risk_factor:.1%}"
                )
            ...
            if (
                ...
                self.logger.info(
                    f"Reducing position size due to high concentration: {concentration_factor:.1%}"
                )
            ...
            if self.current_metrics.ml_risk_score > 0.7:
                ...
                self.logger.info(
                    f"Reducing position size due to high ML risk score: {ml_factor:.1%}"
                )
            ...
            self.logger.info(
                f"Risk - adjusted position size for {symbol}: {base_position_size:.2%} - >  {adjusted_size: .2%}"
            )
    ...
    async def should_allow_trade(
        ...
        try:
            ...
            if not self.current_metrics.within_limits:
                return (
                    False,
                    f"Portfolio risk exceeds limits: {', '.join(self.current_metrics.alerts)}",
                )
            ...
            if position_weight > self.risk_limits.max_position_var:
                return (
                    False,
                    f"Position size {position_weight: .2%} exceeds limit {self.risk_limits.max_position_var: .2%}",
                )

            # Check if adding this trade would exceed concentration limits
            new_concentration = self._calculate_new_concentration(
                symbol, trade_value, portfolio_value
            )
            if new_concentration > self.risk_limits.max_concentration:
                return (
                    False,
                    f"Concentration would exceed limit: {new_concentration:.2%}  >  {self.risk_limits.max_concentration: .2%}",
                )
    ...
    async def get_risk_summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            "utilization": {
                "var_utilization": self.current_metrics.portfolio_var
                / self.risk_limits.max_total_var,
                "cvar_utilization": self.current_metrics.portfolio_cvar
                / self.risk_limits.max_total_cvar,
                "concentration_utilization": self.current_metrics.concentration_risk
                / self.risk_limits.max_concentration,
                "greeks_utilization": self.current_metrics.greeks_risk
                / self.risk_limits.max_greeks_risk,
            },
        }
