...
class RiskDashboard2025:
    ...
    def _calculate_core_risk_metrics(
        ...
        # Extract VaR values
        var_1d = var_suite.results.get(
            "historical_95", var_suite.results.get("monte_carlo_95")
        ).var_value
    ...
    def _calculate_sentiment_risk(self, market_data: dict) -> float:
        ...
        # Safely convert sentiment to float, handling mocks
        try:
            sentiment = float(sentiment)
        except (TypeError, ValueError):
            sentiment = 0.0  # Default to neutral sentiment
    ...
    def _calculate_options_flow_risk(self, market_data: dict) -> float:
        ...
        # Safely convert put_call_ratio to float, handling mocks
        try:
            put_call_ratio = float(put_call_ratio)
        except (TypeError, ValueError):
            put_call_ratio = 1.0  # Default to neutral ratio
...
if __name__ == "__main__":  # Create sample portfolio
    ...
    for metric, data in dashboard_data["risk_metrics"].items():
        print(
            f"{metric.upper()}: ${data['value']:,.0f} ({data['percentage']: .1f}%) - {data['limit_utilization']: .1f}% of limit"
        )
