...
def liquidity_adjusted_var(
    var: float, bid_ask_bps: float = 10.0, slippage_bps: float = 5.0
) -> float:
...
def kupiec_pof(num_exceptions: int, T: int, alpha: float) -> dict[str, float]:
    ...
    a = 1 - alpha
    lr = -2.0 * (
        ((T - num_exceptions) * math.log(1 - a) + num_exceptions * math.log(a))
        - ((T - num_exceptions) * math.log(1 - pi) + num_exceptions * math.log(pi))
    )
    return {"exceptions": num_exceptions, "T": T, "alpha": alpha, "LR_pof": lr}


def rolling_var_exceptions(
    returns: pd.Series, window: int = 250, alpha: float = 0.99
) -> dict[str, float]:
    r = _validate_series(returns)
    ex = 0
...
def apply_stress_scenario(
    returns: pd.Series, scenario: str, portfolio_value: float = 100000.0
) -> float:
...
def calculate_greeks_risk(
    positions: list[dict], underlying_shock: float = 0.03, vol_shock: float = 0.05
) -> dict[str, float]:
...
def calculate_strategy_risk_budget(
    strategies: dict[str, dict], total_var: float, total_cvar: float
