...
class RiskManager: 
    ...
    def generate_risk_report(self)->Dict: 
        ...
        if portfolio_risk.risk_utilization  >  self.risk_params.max_total_risk: 
            report['risk_alerts'].append(f"Portfolio risk utilization ({portfolio_risk.risk_utilization: .1%}) exceeds limit ({self.risk_params.max_total_risk: .1%})")
        ...
        if profit_positions: 
            report['recommendations'].append(f"Consider taking profits on {len(profit_positions)} positions")
