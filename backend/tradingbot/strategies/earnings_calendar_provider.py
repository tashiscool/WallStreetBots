...
class EnhancedEarningsEvent:
    ...
    # IV Analysis
    implied_move: ImpliedMoveData | None = None
    iv_analysis: IVPercentileData | None = None

    # Historical performance
    avg_historical_move: float | None = None  # Average earnings move %
    historical_beat_rate: float | None = None  # % of beats
...
class EarningsCalendarProvider:
    ...
    def __init__(
        ...
        if polygon_api_key:
            try:
                ...
                self.logger.warning(
                    "Polygon.io client not available-install polygon - api - client"
                )
                self.polygon_client = None
        ...
        if alpha_vantage_key:
            try:
                ...
                self.alpha_vantage_client = alphavantage.AlphaVantage(
                    key=alpha_vantage_key
                )
                self.logger.info("Alpha Vantage client initialized")
            except ImportError:
                self.logger.warning(
                    "Alpha Vantage client not available-install alpha - vantage"
                )
                self.alpha_vantage_client = None
            except Exception as e:
        ...
        self.logger.info(
            "EarningsCalendarProvider initialized with external data sources"
        )

    async def get_earnings_calendar(
        self, days_ahead: int = 30
    ) -> list[EnhancedEarningsEvent]:
        ...
        try:
            ...
            if not earnings_events:
                ...
                self.logger.warning(
                    "No earnings events from external sources, using data provider"
                )
        ...
        if self.polygon_client:
            try:
                ...
                if polygon_events:
                    ...
                    self.logger.info(
                        f"Retrieved {len(polygon_events)} earnings events from Polygon.io"
                    )
            except Exception as e:
        ...
        if self.alpha_vantage_client and not earnings_events:
            try:
                ...
                if alpha_events:
                    ...
                    self.logger.info(
                        f"Retrieved {len(alpha_events)} earnings events from Alpha Vantage"
                    )
            except Exception as e:
            ...
            for earnings in earnings_data.get("earnings", []):
                ...
                event = type(
                    ...
                    (),
                    {
                        ...
                        "company_name": earnings.get(
                            "name", earnings.get("symbol", "")
                        ),
                        "earnings_date": datetime.fromisoformat(
                            earnings.get("date", "")
                        ),
    ...
    async def _enhance_earnings_event(self, base_event) -> EnhancedEarningsEvent | None:
        ...
        try:
            ...
            implied_move = await self._calculate_real_implied_move(
                base_event.ticker, base_event.earnings_date, current_price
            )
            ...
            (
                historical_moves,
                beat_rate,
            ) = await self._get_historical_earnings_performance(base_event.ticker)

            # Determine reaction type and recommendations
            reaction_type = self._classify_expected_reaction(
                implied_move, iv_analysis, historical_moves
            )
            recommendations = self._get_strategy_recommendations(
                reaction_type, iv_analysis, implied_move
            )
            ...
            enhanced_event = EnhancedEarningsEvent(
                ...
                avg_historical_move=statistics.mean(historical_moves)
                if historical_moves
                else None,
                historical_beat_rate=beat_rate,
                last_4_reactions=historical_moves[-4:]
                if len(historical_moves) >= 4
                else historical_moves,
    ...
    async def _calculate_real_implied_move(
        ...
        try:
            ...
            atm_calls = [
                ...
                and (opt.expiry == target_expiry or opt.expiry.date() == target_expiry)
            ]
            atm_puts = [
                opt
                for opt in options_chain
                if opt.option_type.lower() == "put"
                and (opt.expiry == target_expiry or opt.expiry.date() == target_expiry)
            ]
            ...
            atm_call = min(
                atm_calls, key=lambda x: abs(float(x.strike) - current_price_float)
            )
            atm_put = min(
                atm_puts, key=lambda x: abs(float(x.strike) - current_price_float)
            )
            ...
            call_mid = (
                ...
                else atm_call.last_price
            )
            put_mid = (
                (atm_put.bid + atm_put.ask) / 2
                if atm_put.bid > 0 and atm_put.ask > 0
                else atm_put.last_price
            )
            ...
            call_spread = (
                float((atm_call.ask - atm_call.bid) / call_mid) if call_mid > 0 else 1.0
            )
            put_spread = (
                float((atm_put.ask - atm_put.bid) / put_mid) if put_mid > 0 else 1.0
            )
            avg_spread = (call_spread + put_spread) / 2

            total_volume = atm_call.volume + atm_put.volume
            confidence = max(
                0.1, min(1.0, (1.0 - avg_spread) * min(1.0, total_volume / 100))
            )
    ...
    def _estimate_implied_move_fallback(
    ...
    async def _analyze_iv_percentiles(self, ticker: str) -> IVPercentileData | None:
        ...
        try:
            ...
            atm_options = [
                opt
                for opt in options_chain
                if abs(float(opt.strike) - current_price) / current_price < 0.05
            ]  # Within 5% of ATM
            ...
            iv_percentile_30d = self._calculate_percentile(
                current_iv, historical_ivs[-30:]
            )
            iv_percentile_90d = self._calculate_percentile(current_iv, historical_ivs)

            historical_mean = statistics.mean(historical_ivs)
            historical_std = (
                statistics.stdev(historical_ivs) if len(historical_ivs) > 1 else 0
            )

            z_score = (
                (current_iv - historical_mean) / historical_std
                if historical_std > 0
                else 0
            )
    ...
    def _calculate_percentile(
        self, value: float, historical_data: list[float]
    ) -> float:
    ...
    def _classify_expected_reaction(
        self,
        ...
        try:
            ...
            if historical_moves:
                avg_historical_move = statistics.mean(
                    [abs(move) for move in historical_moves]
                )
    ...
    def _get_strategy_recommendations(
        self,
        reaction_type: EarningsReactionType,
        iv_analysis: IVPercentileData | None,
        implied_move: ImpliedMoveData | None,
    ) -> list[str]:
            ...
            if reaction_type == EarningsReactionType.HIGH_VOLATILITY:
                if high_iv:
                    recommendations.extend(
                        ["sell_straddle", "iron_condor", "calendar_spread"]
                    )
                else:
                    recommendations.extend(
                        ["buy_straddle", "buy_strangle", "protective_collar"]
                    )
                ...
                if high_iv:
                    recommendations.extend(
                        ["sell_premium", "covered_call", "cash_secured_put"]
                    )
                else:
