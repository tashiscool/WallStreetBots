        ...
        for ticker in ["SPY", "VTI", "QQQ"]:
            try:
                ...
                period_start_price = hist_data.loc[
                    hist_data.index >= start_date.replace(tzinfo=hist_data.index.tz)
                ]["Close"].iloc[0]
                ytd_start_price = hist_data.loc[
                    hist_data.index >= ytd_start.replace(tzinfo=hist_data.index.tz)
                ]["Close"].iloc[0]
                one_year_price = hist_data.loc[
                    hist_data.index >= one_year_ago.replace(tzinfo=hist_data.index.tz)
                ]["Close"].iloc[0]
