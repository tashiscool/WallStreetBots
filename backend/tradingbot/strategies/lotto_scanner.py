        ...
        for ticker in liquid_tickers:
            try:
                ...
                for option_type, target_price in targets:
                    ...
                    # Find closest strike
                    best_strike = options_df.iloc[
                        (options_df["strike"] - target_price).abs().argsort()[:1]
                    ]
        ...
        for event in earnings_events:
            try:
                ...
                for option_type, target_strike, catalyst in plays_to_check:
                    ...
                    # Find best strike
                    best_option = options_df.iloc[
                        (options_df["strike"] - target_strike).abs().argsort()[:1]
                    ]
...
def main():
    ...
    if args.output == "json":
        print(json.dumps([asdict(play) for play in all_plays], indent=2, default=str))
    else:
