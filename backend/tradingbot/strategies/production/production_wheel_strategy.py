...
class WheelPosition:
    ...
    option_premium: Decimal | None = None
...
class ProductionWheelStrategy:
            ...
            try:
                ...
                if position.stage == "cash_secured_put":  # P & L from option premium
                    ...
                    if position.option_premium:
                        ...
                        position.unrealized_pnl = (
                            stock_pnl + option_pnl + position.total_premium_collected
                        )
    ...
    async def _check_position_management(
        ...
        try:
            ...
            if (
                position.stage == "cash_secured_put"
    ...
    async def _analyze_wheel_candidate(self, ticker: str) -> WheelCandidate | None:
        ...
        try:
            ...
            probability_profit = (
                Decimal(str(1.0 - abs(put_analysis.greeks.delta)))
                if put_analysis.greeks
                else Decimal("0.7")
            )
            ...
            return WheelCandidate(
                ...
                put_delta=Decimal(str(put_analysis.greeks.delta))
                if put_analysis.greeks
                else Decimal("-0.25"),
            ...
            if call_analysis and call_analysis.contract.strike >= min_strike:
                return TradeSignal(
                    ...
                    metadata={
                        ...
                        "delta": float(call_analysis.greeks.delta)
                        if call_analysis.greeks
                        else None,
                    },
                )
    ...
    def _create_cash_secured_put_signal(
        ...
        try:
            ...
            self.logger.error(
                f"Error creating cash - secured put signal for {candidate.ticker}: {e}"
            )
            return None
    ...
    async def _update_position_after_trade(
        ...
        try:
            ...
            if stage == "cash_secured_put":  # New cash - secured put position
            ...
            elif stage == "covered_call":  # Add covered call to existing position
                if ticker in self.positions:
                    position = self.positions[ticker]
                    position.stage = "covered_call"
                    position.option_strike = Decimal(str(metadata.get("strike", 0)))
                    position.option_expiry = datetime.fromisoformat(
                        metadata.get("expiry", "")
                    ).date()
        ...
        try:
            ...
            if option_data:
                for contract in option_data:
                    if (
                        contract.strike == position.option_strike
