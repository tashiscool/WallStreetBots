...
from utils.yfinance_hardening import fetch_last_and_prior_close, safe_mid
...
RISK_PCT_DEFAULT = (
    0.10  # 10% of account per signal by default (can set 1.0 to mirror "all - in")
)
...
def bs_call(
    spot: float, strike: float, t: float, r: float, q: float, iv: float
) -> float:
    if min(spot, strike, t, iv) <= 0:
        raise ValueError("positive inputs required")
    d1 = bs_d1(spot, strike, t, r, q, iv)
    d2 = d1 - iv * math.sqrt(t)
    return spot * math.exp(-q * t) * _norm_cdf(d1) - strike * math.exp(
        -r * t
    ) * _norm_cdf(d2)
...
def implied_vol_call(
    ...
    for _ in range(max_iter):
        try:
            d1 = bs_d1(spot, strike, t, r, q, iv)
            d2 = d1 - iv * math.sqrt(t)
            model = spot * math.exp(-q * t) * _norm_cdf(d1) - strike * math.exp(
                -r * t
            ) * _norm_cdf(d2)
...
def fetch_intraday_last_and_prior_close(ticker: str) -> dict[str, float] | None:
    tkr = yf.Ticker(ticker)
...
def get_option_mid_for_nearest_5pct_otm(
    ticker: str, expiry: str, desired_strike: float
) -> dict[str, float] | None:
    tkr = yf.Ticker(ticker)
    ...
    last = (
        float(row["lastPrice"].iloc[0]) if pd.notna(row["lastPrice"].iloc[0]) else 0.0
    )
...
def detect_eod_signal(
    ticker: str, run_lookback: int, run_pct: float, dip_pct: float
) -> DipSignal | None:
...
def monitor_plan(
    ...
    print(
        f"[MONITOR] {ticker} {expiry} C{strike} | entry ${entry_prem: .2f} | target ${target_px: .2f} | stop ${stop_px: .2f}"
    )
    while True:
        ...
        try:
            ...
            last = (
                ...
                if pd.notna(row["lastPrice"].iloc[0])
                else 0.0
            )
            ...
            spot = live["last"]
            dte_days = max(
                (datetime.fromisoformat(expiry).date() - date.today()).days, 1
            )
            t = dte_days / 365.0
            ...
            print(
                f"[{timestamp}] spot={spot: .2f} mid={mid: .2f} (bid={bid * 100: .2f}/ask={ask * 100: .2f}) IV≈{iv: .2%} Δ≈{delta: .2f}"
            )
            ...
            if delta >= delta_target:
                print(
                    f"[DELTA EXIT] Delta target hit: Δ≈{delta:.2f} ≥ {delta_target: .2f}"
                )
                return
            if mid <= stop_px:
...
def write_outputs(
    signals: list[DipSignal], plans: list[OptionPlan], out_prefix: str
) -> None:
    if signals:
...
def run_scan_eod(
    ...
    for t in universe:
        try:
            ...
            if sig:
                print(
                    f"[EOD] {t}: dip {pct(sig.intraday_pct)} after {sig.run_lookback}d run {pct(sig.run_return)} | spot={sig.spot: .2f}"
                )
                ...
                print(
                    ...
                    + str(plan.strike)
                    + " ~5%%OTM | est prem $"
                    + str(plan.premium_est_per_contract)
                    + " | "
    ...
    while True:
        ...
        for t in universe:
            ...
            try:
                ...
                if sig:
                    print(
                        f"[INTRADAY] {t}: dip {pct(sig.intraday_pct)} after {sig.run_lookback}d run {pct(sig.run_return)} | spot={sig.spot: .2f}"
                    )
                    plan = build_exact_plan(
                        t,
                        sig.spot,
...
def run_plan_one(
    ticker: str, spot: float, account_size: float, risk_pct: float, use_chain: bool
) -> None:
    plan = build_exact_plan(
        ticker, spot, account_size, risk_pct, TARGET_DTE_DAYS, OTM_PCT, use_chain
    )
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="WSB Dip - after - Run Scanner / Planner / Monitor"
    )
    ...
    # scan - intraday
    si = sub.add_parser(
        "scan - intraday", help="Intraday scan across universe (polling)"
    )
    ...
    # plan (single ticker)
    pl = sub.add_parser(
        "plan", help="Plan the exact ~5% OTM ~30DTE line for one ticker"
    )
    ...
    mo = sub.add_parser(
        "monitor", help="Monitor a chosen contract for TP / Δ / SL exits"
    )
    ...
    mo.add_argument(
        ...
        required=True,
        type=float,
        help="Entry premium per contract ($)",
    )
