...
def implied_vol_call(
    ...
    for _ in range(max_iter):
        try:
            d1 = bs_d1(spot, strike, t, r, q, iv)
            d2 = d1 - iv * math.sqrt(t)
            model = spot * math.exp(-q * t) * _norm_cdf(d1) - strike * math.exp(-r * t) * _norm_cdf(
                d2
            )
...
def monitor_plan(
    ...
    while True:
        ...
        try:
            ...
            iv = (
                implied_vol_call(
                    market_px=mid_per_share, spot=spot, strike=strike, t=t, r=0.04, q=0.0
                )
                or 0.30
            )
        ...
        try:
            ...
            if sig:
                ...
                plan = build_exact_plan(
                    t, sig.spot, account_size, risk_pct, TARGET_DTE_DAYS, OTM_PCT, use_chain
                )
                print(
                    "      Plan: "
                    + str(t)
...
def parse_args() -> argparse.Namespace:
    ...
    mo.add_argument(
        "--entry - prem", required=True, type=float, help="Entry premium per contract ($)"
    )
...
def main() -> None:
    ...
    if args.cmd == "scan - eod":
        ...
        if not (0 < args.risk_pct <= 1.0):
            ...
            sys.exit(2)
        run_plan_one(
            args.ticker.upper(), args.spot, args.account_size, args.risk_pct, args.use_options_chain
        )
