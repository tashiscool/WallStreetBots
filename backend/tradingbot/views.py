...
def stock_trade(request):
    ...
    # Map order types to model choices
    order_type_map = {
        "market": "M",
        "limit": "L",
        "stop": "S",
    ...
    transaction_type_map = {"buy": "B", "sell": "S"}

    # Validate inputs
    if transaction_side not in transaction_type_map:
        return HttpResponse(
    ...
    order = Order(
        ...
        transaction_type=transaction_type_map[transaction_side],
        status="A",  # Accepted
        client_order_id=result.get("id", ""),
