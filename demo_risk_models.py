...
def demo_var_analysis():
    ...
    for key, result in summary.items():
        ...
        print(
            f"{method.upper(): 12} VaR {conf_pct}%: ${result['var_value']:8,.0f} ({result['var_percentage']: 5.2f}%)"
        )
...
def demo_stress_testing():
    ...
    for scenario_name, result in report.results.items():
        ...
        print(
            f"  {scenario_name: 25} {status: 10} P & L: ${result.portfolio_pnl: 8,.0f} ({pnl_pct: +5.1f}%)"
        )
...
def demo_risk_management_workflow():
    ...
    if stress_report.compliance_status == "NON_COMPLIANT":
        recommendations.append(
            "Improve portfolio diversification - stress tests show compliance issues"
        )
