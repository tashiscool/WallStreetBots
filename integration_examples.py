...
from backend.tradingbot.infra.runtime_safety import (
    Journal,
    assert_ntp_ok,
    ClockDriftError,
)
...
def example_pre_trade_pipeline():
    ...
    try:
        ...
        print(
            f"✓ PDT check passed (equity: ${account_equity:,.0f}, day trades: {pending_day_trades})"
        )
        ...
        print(
            f"✓ All pre-trade checks passed for {order.side} {order.qty} {order.symbol}"
        )
...
def example_wash_sale_tracking():
    ...
    fills = [
        ...
        Fill(
            "AAPL", dt.datetime(2025, 1, 15), "buy", 50, 145.0
        ),  # Replacement within window
    ]
    ...
    for fill in fills:
        if fill.side == "buy":
            ...
            print(
                f"Opened position: {fill.side} {fill.qty} {fill.symbol} @ ${fill.price}"
            )
        else:
            realized, disallowed = wash_engine.realize(fill)
            print(
                f"Closed position: {fill.side} {fill.qty} {fill.symbol} @ ${fill.price}"
            )
...
def example_corporate_actions():
    ...
    actions = [
        CorporateAction(
            "split", pd.Timestamp("2025-01-05"), factor=2.0, amount=0.0
        ),  # 2:1 split
        CorporateAction(
            "div", pd.Timestamp("2025-01-07"), factor=0.0, amount=1.0
        ),  # $1 dividend
    ]
...
def example_universe_management():
    ...
    sp500_members = [
        ...
        Membership(
            "TSLA", dt.date(2020, 12, 21), dt.date(2025, 12, 31)
        ),  # Added Dec 2020
    ...
    for test_date in test_dates:
        ...
        tech_stocks = [
            s for s in members if s in ["AAPL", "MSFT", "TSLA", "FB", "META"]
        ]
