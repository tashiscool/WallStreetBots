...
class NaiveHMMPortfolioUpdate(PortfolioManager):
    def __init__(
        ...
        self.HMM = HMM
        self.num_hidden_states, self.covar_type, self.n_iter = (
            num_hidden_states,
            covar_type,
            n_iter,
        )
    ...
    def rebalance(self):
        ...
        for i in self.portfolio_stocks:
            ...
            current_hmm = self.HMM(
                current_data, self.num_hidden_states, self.covar_type, self.n_iter
            )
        ...
        for stock in buy_list:
            qty = (
                self.total_portfolio_value * (1 - self.buffer) / len(buy_list)
