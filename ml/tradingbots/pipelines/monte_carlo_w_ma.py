...
class MonteCarloMovingAveragePipline(Pipeline):
    def pipeline(self):
        ...
        metric = MovingAverageSharpeRatio(
            self.portfolio_stocks, past_days, max_n, timestep, rf, fetcher
        )
        metric.configure()
        manager = MonteCarloPortfolioUpdate(
            self.portfolio, metric, fetcher, simulation_itr=10000, buffer=0.05
        )
