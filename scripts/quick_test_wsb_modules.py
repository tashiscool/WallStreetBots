...
def generate_summary(import_results, functionality_results):
    ...
    print(
        f"   Successful Imports:         {successful_imports}/{total_modules} ({(successful_imports / total_modules) * 100: .0f}%)"
    )
    print(
        f"   Functional Modules:         {functional_modules}/{total_modules} ({(functional_modules / total_modules) * 100: .0f}%)"
    )
