...
from backend.tradingbot.risk.regulatory_compliance_manager import (
    RegulatoryComplianceManager, RegulatoryAuthority, ComplianceStatus
)
...
async def test_multi_asset_risk():
    ...
    try:
        ...
        print(f"   Asset classes: {set(pos.asset_class for pos in risk_manager.positions.values())}")
...
async def test_automated_rebalancing():
    ...
    try:
        ...
        for asset_info in rebalance_assets:
            print(f"   {asset_info['asset']}: {asset_info['action']} "
