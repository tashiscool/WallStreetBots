...
def test_options_chain_mock():
    ...
    print(
        f"Selected expiry: {best_expiry} ({(datetime.strptime(best_expiry, '%Y-%m-%d').date() - today).days} DTE)"
    )
...
def test_exact_clone_math():
    ...
    print("\nComparison: ")
    print(
        f"  Contract difference: {contracts - original_contracts:,} ({((contracts / original_contracts) - 1) * 100:+.1f}%)"
    )
