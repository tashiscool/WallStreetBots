...
class TestPositionSizingAccuracy(unittest.TestCase):
    ...
    def test_position_sizing_safety_limits(self):
        ...
        self.assertLessEqual(
            sizing["risk_percentage"], self.risk_params.max_single_position_risk * 100
        )
    ...
    def test_position_sizing_input_validation(self):
        ...
        with self.assertRaises(ValueError):
            self.position_sizer.calculate_position_size(
                account_value=-1000, setup_confidence=0.5, premium_per_contract=5.0
            )

        # Zero premium
        with self.assertRaises(ValueError):
            self.position_sizer.calculate_position_size(
                account_value=100000, setup_confidence=0.5, premium_per_contract=0.0
            )

        # Negative premium
        with self.assertRaises(ValueError):
            self.position_sizer.calculate_position_size(
                account_value=100000, setup_confidence=0.5, premium_per_contract=-5.0
            )
...
if __name__ == "__main__":
    run_risk_management_verification_tests()
