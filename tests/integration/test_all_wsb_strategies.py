...
def test_basic_functionality():
    ...
    # Test Swing Trading
    try:
        from swing_trading import SignalType, SwingTradingScanner
        ...
        functionality_results["swing_trading"] = f"❌ FAILED: {e!s}"
        print(f"❌ swing_trading        - Failed: {e!s}")
    ...
    try:
        ...
        functionality_results["spx_credit_spreads"] = f"❌ FAILED: {e!s}"
        print(f"❌ spx_credit_spreads   - Failed: {e!s}")
    ...
    try:
        ...
        functionality_results["earnings_protection"] = f"❌ FAILED: {e!s}"
        print(f"❌ earnings_protection  - Failed: {e!s}")
    ...
    try:
        ...
        functionality_results["index_baseline"] = f"❌ FAILED: {e!s}"
        print(f"❌ index_baseline       - Failed: {e!s}")
...
def generate_comprehensive_report(
    ...
    for module, result in test_results.items():
        if "error" in result:
            ...
            print(
                f"{module: 20} {status} {result['tests_run']} tests, {result['success_rate']: .1f}% success"
            )
    ...
    print(
        f"Functional Modules:      {successful_functionality}/{total_modules} ({(successful_functionality / total_modules) * 100: .1f}%)"
    )
...
def main():
    ...
    try:
        ...
        print(f"\n🚨 CRITICAL ERROR IN TEST RUNNER: {e!s}")
