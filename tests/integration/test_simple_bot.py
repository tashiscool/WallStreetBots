...
async def test_simple_bot():
    ...
    if success:
        ...
        print(f"💰 Account value: ${account_value:,.2f}" if account_value is not None else "💰 Account value: Not available")
        print(f"💵 Balance: ${balance:,.2f}" if balance is not None else "💵 Balance: Not available")
