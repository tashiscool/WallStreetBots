...
from backend.tradingbot.risk.managers.integrated_advanced_risk_manager import (
...
async def test_basic_integration():
    ...
    try:
        ...
        risk_system = await create_integrated_risk_system(
            portfolio_value=100000, regulatory_authority="FCA"
        )
...
async def test_comprehensive_risk_assessment():
    ...
    try:
        ...
        if "var_analysis" in results:
            ...
            for method, data in var_data.items():
                if isinstance(data, dict) and "var_value" in data:
                    print(
                        f"     {method}: ${data['var_value']:,.0f} ({data.get('var_percentage', 0): .2f}%)"
                    )
        ...
        if "stress_testing" in results:
            ...
            print(
                f"   Stress Testing: {stress['scenarios_passed']}/{stress['total_scenarios']} passed"
            )
        ...
        if "ml_agents" in results:
            ...
            print(
                f"   ML Agents: {agents['recommended_action']} (confidence: {agents['confidence']:.3f})"
            )
        ...
        if "compliance" in results:
            ...
            print(
                f"   Compliance: {comp['status']} ({comp['checks_passed']}/{comp['total_checks']} checks)"
            )
