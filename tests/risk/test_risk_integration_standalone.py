...
from backend.tradingbot.risk.managers.risk_integration_manager import (
...
async def test_risk_integration_standalone():
    ...
    try:
        ...
        wsb_result = await risk_aware_wsb.execute_trade("AAPL", "buy", 100, 150.0)
        ...
        hold_result = await risk_aware_wsb.execute_trade("TSLA", "hold", 0, 0.0)
        ...
        disabled_result = await risk_aware_wsb.execute_trade("AAPL", "buy", 1000, 150.0)
        ...
        for i in range(5):
            ...
            print(
                f"   Calculation {i + 1}: VaR={risk_metrics.portfolio_var:.2%}, "
