...
class TestWSBDipDetector:
    ...
    def test_analyze_recent_run_valid(self):
        ...
        run_prices = [
            Decimal(str(100 + i * 4)) for i in range(1, 4)
        ]  # 100 - >  112 (12% run over 3 days)
        ...
        assert run_analysis["run_percentage"] >= 0.12, (
            f"Run percentage was {run_analysis['run_percentage']}"
        )
...
class TestPatternDetectionIntegration:
    ...
    async def test_real_market_scenario_aapl_dip(self):
        ...
        if pattern is not None:
            ...
            assert 0.15 <= metadata["run_percentage"] <= 0.30, (
                f"Run percentage was {metadata['run_percentage']}"
            )
